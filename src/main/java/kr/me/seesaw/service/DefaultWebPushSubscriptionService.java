...
public class DefaultWebPushSubscriptionService implements WebPushSubscriptionService {
    ...
    public WebPushSubscriptionModel subscribe(CreateWebPushSubscriptionCommand command, String userAgent) {
        ...
        Site site = siteRepository.findById(command.getSiteId())
                .orElseThrow(() -> new NoSuchElementException("사이트를 찾을 수 없습니다. id: " + command.getSiteId()));
        User user = userRepository.findByUsername(authentication.getName())
                .orElseThrow(() -> new NoSuchElementException("계정을 찾을 수 없습니다. username: " + authentication.getName()));
